...
Y volvió a pasar. Cuándo puede empezar un item que depende de otro no estaba escrito en ninguna parte, así que cuando `2026-09-04-carga-de-sprint` cambió el fin de una barra de exclusivo a inclusivo no había ningún requisito al que la nueva convención contradijera, y la regla se quedó permitiendo un día de solape. Lo arregló `2026-09-06-un-dependiente-empieza-despues`, que sobre todo la escribió. Dos veces ya que lo que no está en la spec es lo que sobrevive roto a un cambio: por eso lo que esta capacidad cubre se enumera, y por eso las reglas que mueven fechas se escriben aunque parezcan obvias al leerlas en el código.
