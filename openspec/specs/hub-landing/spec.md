...
La pantalla de inicio de la sesión, que responde "qué tengo hoy" antes de pedir en qué aplicación entrar. Cubre el contrato uniforme con que una aplicación reporta su estado —tres cifras, una lista corta con su propia etiqueta y sus avisos—, la rejilla de tarjetas y cómo se representa cada estado de aplicación, la cabecera derivada del estado real, y la tira de avisos agregados. La landing no contiene conocimiento del dominio de ninguna aplicación concreta: registrar una nueva no la modifica.
## Requirements
