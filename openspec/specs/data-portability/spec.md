...
Cubre seis documentos: los roadmaps —en el formato actual y en el heredado—, los temas, las decisiones, los contratos de API, la biblioteca de modelos y las áreas de enlaces. Cada uno es independiente: un fichero de decisiones nunca lleva roadmaps ni al revés, y una aplicación que rechaza el documento de otra dice de cuál es.

Lo que un documento abarca lo decide cada aplicación, y no es lo mismo en las cuatro. Roadmaps y API Hub exportan la unidad de trabajo que tienen abierta —un roadmap, un contrato—, Links Hub exporta un área, y Decisions exporta el conjunto entero porque no tiene unidad abierta que exportar. La biblioteca de modelos es el segundo documento de API Hub, y existe porque es lo único que la mueve entre máquinas.
