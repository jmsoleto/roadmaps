...
- [x] 7.5 Comprobar el comportamiento en Tauri (WKWebView), donde el `scrollLeft` inicial y el `bind:this` del contenedor deben funcionar igual que en el navegador — **cerrada sin comprobar**: el change `2026-08-10-eliminar-empaquetado-escritorio` retiró Tauri del proyecto, así que este entorno ya no existe y la comprobación dejó de tener objeto. La web y la PWA son la única vía de distribución
